...
const Koios = {
  ...
  async getTip() {
    let response;
    try {
      response = await axios.get("https://api.koios.rest/api/v1/tip", {
        headers: {
  ...
  async getParameters(epoch) {
    if (epoch === undefined) {
      ...
      if (!tip) {
        console.log("Could not fetch tip from Koios. Returning default parameters.");
    ...
    let response;
    try {
      response = await axios.get("https://api.koios.rest/api/v1/epoch_params?_epoch_no=" + epoch, {
        headers: {
          authorization: `Bearer ${koios_key}`,
        },
      });
    } catch (e) {
      console.log("Could not fetch parameters from Koios. Returning default parameters.");
      return this.defaultParameters;
    }
    ...
    Vue.mixin({
      ...
      methods: {
        formatAda(value) {
          ...
          return the_number.toLocaleString(undefined, this.cardano.lovelace_format) + ` ₳`;
        },
        toAda(lovelace) {
        ...
        async checkNetwork(network_id) {
          ...
          console.log(`Wallet Network ID: ${wallet_network}`, typeof wallet_network);
        ...
        async checkBalance(asset) {
          try {
            const balance = CSL.Value.from_hex(await this.cardano.Wallet.getBalance());

            if (asset) {
              const policy = CSL.ScriptHash.from_bytes(this.fromHex(asset.policy_id));
              const asset_name = CSL.AssetName.new(this.fromHex(asset.asset_id));
              if (balance.multiasset() === undefined) {
                return 0;
              } else {
                return balance.multiasset().get_asset(policy, asset_name).to_str();
              }
            } else {
        ...
        async getTokens(policy_id) {
          try {
            ...
            const ScriptHash = CSL.ScriptHash.from_bytes(this.fromHex(policy_id));
        ...
        async getStakeCbor() {
          const rewardAddresses = await this.cardano.Wallet.getRewardAddresses();
          return rewardAddresses[0];
        },
        async getStakeKey() {
          const stakeAddressCbor = await this.getStakeCbor();
          const stakeAddress = CSL.Address.from_bytes(this.toUint8Array(stakeAddressCbor));
        ...
        async getUTxO(pagination) {
          ...
          walletUTxO.map((utxo) => {
            UTxO.add(CSL.TransactionUnspentOutput.from_bytes(this.fromHex(utxo)));
          });
          return UTxO;
        },
        async prepareTransaction() {
          ...
          const txBuilderConfig = CSL.TransactionBuilderConfigBuilder.new()
            ...
            .coins_per_utxo_byte(CSL.BigNum.from_str(protocolParameters.costPerWord.toString()))
            .pool_deposit(CSL.BigNum.from_str(protocolParameters.poolDeposit.toString()))
            .key_deposit(CSL.BigNum.from_str(protocolParameters.keyDeposit.toString()))
            .max_value_size(protocolParameters.maxValSize)
            .max_tx_size(protocolParameters.maxTxSize)
            .ex_unit_prices(
              CSL.ExUnitPrices.new(
                CSL.UnitInterval.new(CSL.BigNum.from_str("1"), CSL.BigNum.from_str("1")),
                CSL.UnitInterval.new(CSL.BigNum.from_str("1"), CSL.BigNum.from_str("1"))
              )
            )
            .build();
        ...
        async makeTransaction(recipients, metadata, ttl, dataHash) {
          ...
          (await this.cardano.Wallet.getUtxos()).map((utxo) =>
            inputs.add(CSL.TransactionUnspentOutput.from_bytes(this.fromHex(utxo)))
          );
          ...
          recipients.forEach((recipient) => {
            ...
            if (recipient.mintedAssets !== undefined && recipient.mintedAssets.length) {
            ...
            if (recipient.assets !== undefined && recipient.assets.length > 0) {
              ...
              recipient.assets.forEach((entry) => {
                ...
                const assetName = CSL.AssetName.new(this.fromAscii(entry.asset));
              ...
              if (lovelace > 0) {
                output = CSL.TransactionOutputBuilder.new()
                  ...
                  .next()
                  .with_coin_and_asset(CSL.BigNum.from_str(lovelace.toString()), multiasset)
                  .build();
              } else {
                output = CSL.TransactionOutputBuilder.new()
                  .with_address(ReceiveAddress)
          ...
          if (minting) {
            ...
            MintingPolicies.forEach((scriptHash) => {
              NativeScripts.add(CSL.NativeScript.from_bytes(this.fromHex(scriptHash)));
            });
            ...
            Object.entries(assetsDict).map(([name, asset]) => {
              try {
                ...
                if (asset.quantity < 0) {
                  ...
                  assetQty = CSL.Int.new(CSL.BigNum.from_str(asset.quantity.toString()));
                }
                ...
                console.error("Error attempting to add a minted asset!", e, asset, name);
              }
            });
          }
          ...
          if (dataHash) {
            const auxDataHash = CSL.AuxiliaryDataHash.from_bytes(this.fromHex(dataHash));
        ...
        async submitTransaction(tx, witnesses) {
          try {
            ...
            for (let witness of witnesses) {
              const addWitnesses = CSL.TransactionWitnessSet.from_bytes(this.fromHex(witness));
            ...
            const signedTx = await CSL.Transaction.new(transaction.body(), totalWitnesses);

            try {
              return await this.cardano.Wallet.submitTx(this.toHex(signedTx.to_bytes()));
            } catch (e) {
